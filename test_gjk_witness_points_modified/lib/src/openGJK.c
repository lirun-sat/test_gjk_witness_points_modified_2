...
inline static void S1D(struct simplex * s, double *vv) {
  ...
  /* Project origin onto the 1D simplex - line */
  for ( int ii = 0; ii < 3; ++ii ) {
    po[ii] = dotProduct(b, t) / (dotProduct(t, t)) * (b[ii] - a[ii]) + b[ii];
  }
  po_indexI = po[indexI];
  ...
  /* Compute signed determinants */
  det_ap = a[indexI] - po_indexI;  // C_2
  det_pb = po_indexI - b[indexI];  // C_1
  ...
  /* Compare signs of AB and auxiliary simplices */
  FacetsTest[0] = SAMESIGN(nu_max, det_ap);  // nu_max, C_2
  FacetsTest[1] = SAMESIGN(nu_max, det_pb);  // nu_max, C_1
  ...
  if (FacetsTest[0] + FacetsTest[1] == 2) {
    /* The origin is between A and B */
    s->lambdas[0] = det_pb / nu_max;  // C_1 / nu_max
    s->lambdas[1] = det_ap / nu_max;  // C_2 / nu_max
    s->wids[0] = 0;
    s->wids[1] = 1;
    s->nvrtx = 2;
  }
